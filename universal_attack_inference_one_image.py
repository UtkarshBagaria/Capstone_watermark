...
def parse(args=None):
    with open(join('/content/CMUA-Watermark/setting.json'), 'r') as f:
...
if __name__ == "__main__":
    args_attack = parse()
    print(args_attack)
    os.system('cp -r /content/CMUA-Watermark/results {}/results{}'.format(args_attack.global_settings.results_path, args_attack.attacks.momentum))
    print("experiment dir is created")
    os.system('cp /content/CMUA-Watermark/setting.json {}'.format(os.path.join(args_attack.global_settings.results_path, 'results{}/content/CMUA-Watermark/setting.json'.format(args_attack.attacks.momentum))))
    ...
    for idx, (img_a, att_a, c_org) in enumerate(test_dataloader):
        ...
        ############# 保存图片做指标评测 #############
        # 保存原图
        out_file = '/content/CMUA-Watermark/demo_results/AttGAN_original.jpg'
        vutils.save_image(
            img_a.cpu(), out_file,
        ...
        for j in range(len(samples)-2):
            # 保存对抗样本生成的图片
            out_file = '/content/CMUA-Watermark/demo_results/AttGAN_advgen_{}.jpg'.format(j)
            vutils.save_image(samples[j+2], out_file, nrow=1, normalize=True, range=(-1., 1.))
            # 保存原图生成的图片
            out_file = '/content/CMUA-Watermark/demo_results/AttGAN_gen_{}.jpg'.format(j)
    ...
    for idx, (img_a, att_a, c_org) in enumerate(test_dataloader):
        ...
        ############# 保存图片做指标评测 #############
        # 保存原图
        out_file = '/content/CMUA-Watermark/demo_results/stargan_original.jpg'
        vutils.save_image(img_a.cpu(), out_file, nrow=1, normalize=True, range=(-1., 1.))
        for j in range(len(x_fake_list)):
            # 保存原图生成图片
            gen_noattack = x_noattack_list[j]
            out_file = '/content/CMUA-Watermark/demo_results/stargan_gen_{}.jpg'.format(j)
            vutils.save_image(gen_noattack, out_file, nrow=1, normalize=True, range=(-1., 1.))
            # 保存对抗样本生成图片
            gen = x_fake_list[j]
            out_file = '/content/CMUA-Watermark/demo_results/stargan_advgen_{}.jpg'.format(j)
    ...
    for idx, (img_a, att_a, c_org) in enumerate(test_dataloader):
        ...
        ############# 保存图片做指标评测 #############
        # 保存原图
        out_file = '/content/CMUA-Watermark/demo_results/attentiongan_original.jpg'
        vutils.save_image(img_a.cpu(), out_file, nrow=1, normalize=True, range=(-1., 1.))
        for j in range(len(x_fake_list)):
            # 保存原图生成图片
            gen_noattack = x_noattack_list[j]
            out_file = '/content/CMUA-Watermark/demo_results/attentiongan_gen_{}.jpg'.format(j)
            vutils.save_image(gen_noattack, out_file, nrow=1, normalize=True, range=(-1., 1.))
            # 保存对抗样本生成图片
            gen = x_fake_list[j]
            out_file = '/content/CMUA-Watermark/demo_results/attentiongan_advgen_{}.jpg'.format(j)
            vutils.save_image(gen, out_file, nrow=1, normalize=True, range=(-1., 1.))
        break
    ...
    for idx, (img_a, att_a, c_org) in enumerate(test_dataloader):
        ...
        with torch.no_grad():
            ...
            ############# 保存图片做指标评测 #############
            # 保存原图
            out_file = '/content/CMUA-Watermark/demo_results/HiSD_original.jpg'
            vutils.save_image(img_a.cpu(), out_file, nrow=1, normalize=True, range=(-1., 1.))
            
            out_file = '/content/CMUA-Watermark/demo_results/HiSD_gen.jpg'
            vutils.save_image(gen_noattack, out_file, nrow=1, normalize=True, range=(-1., 1.))
            # 保存对抗样本生成图片
            gen = x_fake_list[j]
            out_file = '/content/CMUA-Watermark/demo_results/HiSD_advgen.jpg'
            vutils.save_image(gen, out_file, nrow=1, normalize=True, range=(-1., 1.))
        break
