...
def init_attGAN(args_attack):
    with open(join('/content/CMUA-Watermark/AttGAN/output', args_attack.AttGAN.attgan_experiment_name, 'setting.txt'), 'r') as f:
    ...
    attgan = AttGAN(args)
    attgan.load(find_model(join('/content/CMUA-Watermark/AttGAN/output', args.experiment_name, 'checkpoint'), args.load_epoch))
    attgan.eval()
    return attgan, args
